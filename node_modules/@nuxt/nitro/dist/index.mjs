import { resolve, dirname, relative, extname, join } from 'pathe';
import { resolvePath as resolvePath$1, logger } from '@nuxt/kit';
import * as rollup from 'rollup';
import fse from 'fs-extra';
import { genDynamicImport, genImport, genObjectFromRawEntries, genArrayFromRaw, genString } from 'knitwork';
...
function getAssetProd(assets2) {
  return `
const _assets = ${genObjectFromRawEntries(Object.entries(assets2).map(([id, asset]) => [normalizeKey(id), {
...
const middleware = ${genArrayFromRaw(middleware2.map((m) => ({
          route: JSON.stringify(m.route),
          handle: getImportId(m.handle),
          lazy: m.lazy || true,
          promisify: m.promisify !== void 0 ? m.promisify : true
        })))};
...
async function resolveMiddleware(nuxt) {
  ...
  for (let m of nuxt.options.serverMiddleware) {
    ...
    if (typeof handle !== "string" || typeof route !== "string") {
      ...
      middleware.push({
        ...m,
        handle: await resolvePath$1(handle),
        route
      });
    }
  }
  ...
  return {
    ...
    hooks: {
      ...
      async "nitro:compiled"({ output }) {
        await promises.writeFile(resolve(output.publicDir, "sw.js"), `self.importScripts(${genString(joinURL(baseURL, "_server/index.mjs"))});`, "utf8");
        ...
        logger.info("Ready to deploy to static hosting:", prettyPath(output.publicDir));
      }
    }
  };
});
